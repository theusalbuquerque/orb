...
def _remote_plan(a: dict[str, Any], b: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    # 0) Impact-aligned intro bed.
    #
    # This is deliberately not a long DJ_BLEND. B's opening arrangement is allowed to live
    # quietly under A while A remains the foreground track; the clock is B's first meaningful
    # impact, which is aligned to A's content end. The recipe is available only when B's pre-impact
    # material is genuinely bed-like (low vocal activity), the pair is harmonically/temporally
    # compatible, and A releases late enough that the bed does not become an early takeover.
    intro_duration = b_impact - b_start
    intro_tempo_ratio = abs(b_bpm / a_bpm - 1.0) if a_bpm > 0.0 and b_bpm > 0.0 else 1.0
    if (
        a_end > 0.0
        and b_end > b_start
        and 12.0 <= intro_duration <= 68.0
        and key_evidence
        and key_fit >= 0.58
        and conf >= 0.30
        and tempo >= 0.62
        and intro_tempo_ratio <= 0.045
    ):
        b_beat = 60.0 / b_bpm if b_bpm > 0.0 else 0.5
        impact_anchor, impact_structure = _structural_snap(
            b,
            b_impact,
            max(b_start, b_impact - 2.5 * b_beat),
            min(b_end - 0.25, b_impact + 2.5 * b_beat),
            b_beat,
        )
        ...
        intro_peak_vocal = _max_window_mean(
            b_vocal,
            b_start,
            impact_anchor,
            2.0,
        ...
        impact_pre_energy = _mean_window(
            b_energy,
            max(b_start, impact_anchor - 6.0),
            impact_anchor,
            0.0,
        )
        impact_post_energy = _mean_window(
            b_energy,
            impact_anchor,
            min(b_end, impact_anchor + 4.0),
            impact_pre_energy,
        )
        b_low = _low_curve(b)
        impact_pre_low = _mean_window(
            b_low,
            max(b_start, impact_anchor - 6.0),
            impact_anchor,
            0.0,
        )
        impact_post_low = _mean_window(
            b_low,
            impact_anchor,
            min(b_end, impact_anchor + 4.0),
            impact_pre_low,
        ...
        impact_lift = _clamp(
            (impact_post_energy - impact_pre_energy)
            + 0.75 * (impact_post_low - impact_pre_low),
            0.0,
            1.0,
        )

        # For a long bed, preserve A's original tempo. Only B is gently brought onto A's grid.
        bed_in_rate = _clamp(a_bpm / b_bpm, 0.96, 1.04)
        bed_out_rate = 1.0
        desired_span = (impact_anchor - b_start) / max(bed_in_rate, 1e-6)
        desired_start = a_end - desired_span
        a_beat = 60.0 / a_bpm if a_bpm > 0.0 else 0.5
        outgoing_candidates = _timing_candidates(
            a,
            desired_start,
            max(0.0, desired_start - 4.0 * a_beat),
            min(a_end - 0.50, desired_start + 4.0 * a_beat),
            a_beat,
            "",
            limit=8,
        )

        best_bed: dict[str, Any] | None = None
        best_bed_score = float("-inf")
        for a_start, a_structure, a_kind in outgoing_candidates:
            overlap_seconds = a_end - a_start
            if overlap_seconds < 12.0 or overlap_seconds > 68.0:
                continue

            cue = impact_anchor - overlap_seconds * bed_in_rate
            skipped = cue - b_start
            if cue < b_start - 0.05 or skipped > 8.0:
                continue
            cue = max(b_start, cue)
            bed_media_seconds = impact_anchor - cue
            if bed_media_seconds < 12.0:
                continue
            ...
            release_fraction = _clamp(
                (a_release - a_start) / max(overlap_seconds, 1e-6),
                0.0,
                1.0,
            )
            if release_fraction < 0.70:
                continue

            full_intro_fit = 1.0 - _clamp(skipped / 8.0, 0.0, 1.0)
            bed_score = (
                0.22
                + 0.13 * tempo
                + 0.15 * key_fit
                + 0.07 * conf
                + 0.12 * (1.0 - intro_vocal)
                + 0.09 * impact_lift
                + 0.08 * a_structure
                + 0.05 * (1.0 - overlap_vocal_clash)
                + 0.05 * full_intro_fit
                + (0.07 if protected else 0.02)
            )
            if bed_score <= best_bed_score:
                continue

            best_bed_score = bed_score
            best_bed = {
                "start": a_start,
                "cue": cue,
                ...
                "outgoingStructure": a_structure,
                "outgoingAnchor": a_kind,
                "vocalClash": overlap_vocal_clash,
                "energyFit": energy_fit,
                "fullIntroFit": full_intro_fit,
            }

        impact_is_structural = impact_structure >= 0.70
        intro_is_clean = intro_vocal <= 0.36 and intro_peak_vocal <= 0.60
        impact_is_meaningful = impact_is_structural or impact_lift >= 0.12
        if best_bed is not None and intro_is_clean and impact_is_meaningful:
            bed_span = float(best_bed["overlap"])
            release_fraction = float(best_bed["releaseFraction"])
            actual_beats = max(1, int(round(bed_span / max(a_beat, 1e-6))))
            candidates.append(_candidate_plan(
                "INTRO_BED",
                best_bed_score,
                "server-impact-aligned-intro-bed",
                transitionStart=round(float(best_bed["start"]), 4),
                transitionEnd=round(a_end, 4),
                incomingCueTime=round(float(best_bed["cue"]), 4),
                incomingHandoffTime=round(impact_anchor, 4),
                outgoingPlaybackRate=bed_out_rate,
                incomingPlaybackRate=round(bed_in_rate, 5),
                transitionBeats=actual_beats,
                requestedTransitionBeats=actual_beats,
                handoffFraction=round(_clamp(release_fraction, 0.76, 0.985), 4),
                bassSwap=bool(_low_curve(a) and _low_curve(b)),
                bassSwapFraction=round(_clamp(release_fraction, 0.74, 0.975), 4),
                filterSweep=0.0,
                keyCompatibility=round(key_fit, 4),
                tempoCompatibility=round(tempo, 4),
                phraseAlignment=round(
                    min(float(best_bed["outgoingStructure"]), max(0.45, impact_structure)),
                    4,
                ),
                overlapVocalClash=round(float(best_bed["vocalClash"]), 4),
                energyCompatibility=round(float(best_bed["energyFit"]), 4),
                pairCompatibility=round(_clamp(best_bed_score, 0.0, 1.0), 4),
                spanCompatibility=round(float(best_bed["fullIntroFit"]), 4),
                outgoingAnchor=str(best_bed["outgoingAnchor"]),
                incomingAnchor="intro-impact",
                introBedSeconds=round(bed_span, 4),
                impactTime=round(impact_anchor, 4),
                gainEnvelope=[],
            ))
