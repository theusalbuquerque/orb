...
AnalysisResult AnalyzeAudio(const std::vector<float>& s,double sr,double duration){AnalysisResult r; if(s.empty()||sr<=0)return r;r.duration=duration>0?duration:s.size()/sr;
 ...
 static const char* names[]={"C","C#","D","D#","E","F","F#","G","G#","A","A#","B"};int best=0;for(int i=1;i<12;++i)if(chroma[i]>chroma[best])best=i;double total=std::accumulate(chroma.begin(),chroma.end(),0.0);r.key=std::string(names[best])+" major";r.key_confidence=total>0?clamp01(chroma[best]/total*4):0;
