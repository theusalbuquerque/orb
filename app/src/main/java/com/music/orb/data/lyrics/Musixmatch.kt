...
/** Line-synced lyrics from Musixmatch's web-client API. */
object Musixmatch {
    private const val BASE = "https://apic.musixmatch.com/ws/1.1"
    private val SIGNING_SECRET: String get() = com.music.orb.BuildConfig.MUSIXMATCH_SIGNING_SECRET
    private val tokenMutex = Mutex()
    private val cachedToken = AtomicReference<String?>(null)

    suspend fun lyrics(title: String, artist: String, durationMs: Long): List<LyricLine>? = withContext(Dispatchers.IO) {
    ...
    private suspend fun bestTrack(title: String, artist: String, seconds: Int): Track? =
        searchTrack(title, artist)?.maxByOrNull { score(it, title, artist, seconds) }
    ...
    private suspend fun searchTrack(title: String, artist: String): List<Track>? {
        ...
        return runCatching { lyricsJson.decodeFromString<Envelope<TrackSearchBody>>(response) }
    ...
    private suspend fun fetchSubtitle(trackId: Long): String? {
        ...
        return runCatching { lyricsJson.decodeFromString<Envelope<SubtitleBody>>(response) }
            .getOrNull()?.message?.body?.subtitle?.subtitleBody
    }

    private fun subtitleToLrc(subtitleBody: String): String {
        val lines = runCatching { lyricsJson.decodeFromString<List<SubtitleLine>>(subtitleBody) }.getOrNull() ?: return ""
        return buildString {
            for (line in lines) {
                ...
                appendLine("[" + "%02d:%02d.%03d".format(Locale.US, minutes, seconds, millis) + "]" + line.text)
            }
        }.trim()
    }
    ...
    private fun fetchToken(): String? {
        if (SIGNING_SECRET.isBlank()) return null
        val url = "$BASE/token.get".toHttpUrl().newBuilder()
            .addQueryParameter("app_id", "web-desktop-app-v1.0")
            .build()
        val body = lyricsGet(sign(url.toString())) ?: return null
        return runCatching { lyricsJson.decodeFromString<Envelope<TokenBody>>(body) }
    ...
    @Serializable private data class Envelope<T>(val message: Message<T>)
    @Serializable private data class Message<T>(val header: Header, val body: T? = null)
    @Serializable private data class Header(@SerialName("status_code") val statusCode: Int = 0)
    @Serializable private data class TokenBody(@SerialName("user_token") val userToken: String)
    @Serializable private data class TrackSearchBody(@SerialName("track_list") val trackList: List<TrackWrapper> = emptyList())
    @Serializable private data class TrackWrapper(val track: Track)
    @Serializable private data class Track(
    ...
    @Serializable private data class SubtitleBody(val subtitle: Subtitle? = null)
    @Serializable private data class Subtitle(@SerialName("subtitle_body") val subtitleBody: String)
    @Serializable private data class SubtitleLine(val text: String, val time: SubtitleTime)
    @Serializable private data class SubtitleTime(val total: Double)
}
