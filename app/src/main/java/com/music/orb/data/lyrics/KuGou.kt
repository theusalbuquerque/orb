...
/** Line-synced lyrics from KuGou's public endpoints. */
object KuGou {
    ...
    private fun download(id: String, accessKey: String): String? {
        ...
        val response = runCatching { lyricsJson.decodeFromString<DownloadResponse>(body) }.getOrNull()
            ?: return null
        val decoded = runCatching { Base64.getDecoder().decode(response.content).toString(Charsets.UTF_8) }.getOrNull()
            ?: return null
    ...
    private data class SearchSongResponse(val data: Data? = null) {
        @Serializable data class Data(val info: List<Info> = emptyList())
        @Serializable data class Info(val hash: String, val duration: Int = -1)
    }
    @Serializable private data class SearchLyricsResponse(val candidates: List<Candidate> = emptyList())
    @Serializable private data class Candidate(val id: String, val accesskey: String)
    @Serializable private data class DownloadResponse(val content: String = "")
}
