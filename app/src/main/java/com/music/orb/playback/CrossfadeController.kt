...
class CrossfadeController(
    ...
    private fun considerSmartTransition(duration: Long) {
        ...
        if (currentItem.albumSequential && nextItem.albumSequential) {
            AppSettings.smartTransitionWindow.value = null
    ...
    private fun begin(
        ...
        this.loopRepeatsPlanned = this.loopRepeatsRemaining        loopSeekBlockedUntilMs = 0L
    ...
    private fun finish() {
        ...
        val winner = incoming
        val incomingRateToRelease = if (render.automix25 && usesTempoBridgeStyle()) {
            1.0
        } else {
        ...
        if (handedOff) {
            ...
            incoming?.let {
                ...
                if (kotlin.math.abs(incomingRateToRelease - 1.0) < RATE_EPSILON) {
            ...
            outgoing?.run {
                ...
                volume = 1f
                setPlaybackSpeed(AppSettings.playbackSpeed.value)
            }
            incoming?.let(::retire)
        }
        ...
        phase = Phase.IDLE
        if (completedSmartMix && winner != null && kotlin.math.abs(incomingRateToRelease - 1.0) >= RATE_EPSILON) {
    ...
    private fun releasePlaybackRateGradually(player: ExoPlayer, relativeStart: Double) {
        ...
        scope.launch {
            repeat(TEMPO_RELEASE_STEPS) { index ->
                ...
                player.setPlaybackSpeed((AppSettings.playbackSpeed.value * relative).toFloat())
    ...
    /**
     * Keep both decks on the server's meeting tempo until the musical handoff, then return B to
     * its native tempo progressively. This is time-stretch with pitch preserved, not pitch shift.
     */
    private fun rideTempoBridge(progress: Float, out: ExoPlayer, into: ExoPlayer) {
        ...
        val handoff = render.handoffFraction.toFloat().coerceIn(0.42f, 0.90f)
        val releaseStart = (handoff + TEMPO_RELEASE_AFTER_HANDOFF).coerceAtMost(0.92f)
        val release = smoothStep(releaseStart, 1f, progress.coerceIn(0f, 1f)).toDouble()
        val outRate = outgoingPlaybackRate.takeIf { it.isFinite() && it > 0.0 } ?: 1.0
        val meetingInRate = incomingPlaybackRate.takeIf { it.isFinite() && it > 0.0 } ?: 1.0
        val inRate = meetingInRate + (1.0 - meetingInRate) * release
