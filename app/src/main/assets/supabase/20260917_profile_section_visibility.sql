...
--       - favorite albums
--       - favorite songs
--   * Hidden means hidden from other people, not from the profile owner.
...
-- Existing users get the new profile sections enabled. If this migration is
-- re-run later, already-saved choices are preserved because only NULL fields
-- are initialized here.
update public.profiles
set following_visibility = coalesce(following_visibility, 'everyone'),
    followers_visibility = coalesce(followers_visibility, 'everyone'),
    play_count_visibility = coalesce(play_count_visibility, 'everyone'),
    favorite_artists_visibility = case
        when lower(coalesce(favorite_artists_visibility, favorite_content_visibility, 'everyone'))
             in ('everyone', 'public', 'anyone') then 'everyone'
        else 'nobody'
    end,
    favorite_albums_visibility = case
        when lower(coalesce(favorite_albums_visibility, favorite_content_visibility, 'everyone'))
             in ('everyone', 'public', 'anyone') then 'everyone'
        else 'nobody'
    end,
    favorite_songs_visibility = case
        when favorite_songs_visibility is not null then
            case when lower(favorite_songs_visibility) in ('everyone', 'public', 'anyone') then 'everyone' else 'nobody' end
...
-- compatibility field privacy-safe: it is public only while all three favorite
-- sections are public.
update public.profiles
set favorite_content_visibility = case
    when favorite_artists_visibility = 'everyone'
     and favorite_albums_visibility = 'everyone'
     and favorite_songs_visibility = 'everyone'
    then 'everyone'
    else 'nobody'
end;
...
alter table public.profiles
    add constraint profiles_following_visibility_check
        check (following_visibility in ('everyone', 'nobody')),
    add constraint profiles_followers_visibility_check
        check (followers_visibility in ('everyone', 'nobody')),
    add constraint profiles_play_count_visibility_check
        check (play_count_visibility in ('everyone', 'nobody')),
    add constraint profiles_favorite_artists_visibility_check
        check (favorite_artists_visibility in ('everyone', 'nobody')),
    add constraint profiles_favorite_albums_visibility_check
        check (favorite_albums_visibility in ('everyone', 'nobody')),
    add constraint profiles_favorite_songs_visibility_check
        check (favorite_songs_visibility in ('everyone', 'nobody'));
...
begin
    ...
    return coalesce(audience, 'nobody') in ('everyone', 'public', 'anyone');
end;
$$;
...
-- Do not expose the entire follow graph directly anymore. Public profile lists
...
-- Counts are aggregate profile metadata, but each field is independently
-- redacted to zero when the profile owner hides it. The UI also hides the field,
...
-- an older build receives favorite data only when all three categories are
-- public, avoiding a privacy leak through its old unified UI.
